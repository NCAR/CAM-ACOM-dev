module test_cloud_aqueous_chemistry_mod
  ...
  character(len=*), parameter :: data_file = '../test/chemistry/data/QPC6-f10_f10_mg37.cam.h1i.0001-01-05-61200.nc'
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine test_as_primary_rank()
    ...
    if (.not. compare_outputs(chem_args, expected_outputs)) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  function get_expected_outputs() result(chem_args)
    ...
    do j = 1, n_modes
      ...
      do i = 1, size(chem_args)
        chem_args(i)%aqso4(:,j) = temp1d((i-1)*n_columns_per_rank+1:min(i*n_columns_per_rank,size(temp2d, dim=1)))
      end do
      call read_and_condense_columns(file, 'cloud_aqh2so4_'//trim(adjustl(index_string))//'_out', temp1d)
      do i = 1, size(chem_args)
        chem_args(i)%aqh2so4(:,j) = temp1d((i-1)*n_columns_per_rank+1:min(i*n_columns_per_rank,size(temp2d, dim=1)))
      end do
    end do
    call read_and_condense_columns(file, 'cloud_aqso4_h2o2_out', temp1d)
    do i = 1, size(chem_args)
      chem_args(i)%aqso4_h2o2(:) = temp1d((i-1)*n_columns_per_rank+1:min(i*n_columns_per_rank,size(temp2d, dim=1)))
    end do
    call read_and_condense_columns(file, 'cloud_aqso4_o3_out', temp1d)
    do i = 1, size(chem_args)
      chem_args(i)%aqso4_o3(:) = temp1d((i-1)*n_columns_per_rank+1:min(i*n_columns_per_rank,size(temp2d, dim=1)))
    end do
    deallocate(file)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine initialize_args(chem_args)
    ...
    do i = 1, size(chem_args)
      chem_args(i)%ncol = min(16, n_lat*n_lon - (i-1)*16)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  logical function compare_outputs(calculated, expected) result(passed)
    ...
    do i = 1, size(calculated)
      do j = 1, calculated(i)%ncol
        do k = 1, n_layers
          if (.not. check_close(calculated(i)%pres(j,k), expected(i)%pres(j,k))) then
            print *, 'pres mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          if (.not. check_close(calculated(i)%pdel(j,k), expected(i)%pdel(j,k))) then
            print *, 'pdel mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          if (.not. check_close(calculated(i)%tfld(j,k), expected(i)%tfld(j,k))) then
            print *, 'tfld mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          if (.not. check_close(calculated(i)%mbar(j,k), expected(i)%mbar(j,k))) then
            print *, 'mbar mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          if (.not. check_close(calculated(i)%lwc(j,k), expected(i)%lwc(j,k))) then
            print *, 'lwc mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          if (.not. check_close(calculated(i)%cldfrc(j,k), expected(i)%cldfrc(j,k))) then
            print *, 'cldfrc mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          if (.not. check_close(calculated(i)%cldnum(j,k), expected(i)%cldnum(j,k))) then
            print *, 'cldnum mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          if (.not. check_close(calculated(i)%xhnm(j,k), expected(i)%xhnm(j,k))) then
            print *, 'xhnm mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          do l = 1, n_invariants
            if (.not. check_close(calculated(i)%invariants(j,k,l), expected(i)%invariants(j,k,l))) then
              print *, 'invariants mismatch at column ', (i-1)*j, ' and layer ', k, ' and invariant ', l
              passed = .false.
            end if
          end do
          do l = 1, n_species
            if (.not. check_close(calculated(i)%qcw(j,k,l), expected(i)%qcw(j,k,l))) then
              print *, 'qcw mismatch at column ', (i-1)*j, ' and layer ', k, ' and species ', l
              passed = .false.
            end if
            if (.not. check_close(calculated(i)%qin(j,k,l), expected(i)%qin(j,k,l))) then
              print *, 'qin mismatch at column ', (i-1)*j, ' and layer ', k, ' and species ', l
              passed = .false.
            end if
          end do
          if (.not. check_close(calculated(i)%xphlwc(j,k), expected(i)%xphlwc(j,k))) then
            print *, 'xphlwc mismatch at column ', (i-1)*j, ' and layer ', k
            passed = .false.
          end if
          do l = 1, n_modes
            if (.not. check_close(calculated(i)%aqso4(j,l), expected(i)%aqso4(j,l))) then
              print *, 'aqso4 mismatch at column ', (i-1)*j, ' and mode ', l
              passed = .false.
            end if
            if (.not. check_close(calculated(i)%aqh2so4(j,l), expected(i)%aqh2so4(j,l))) then
              print *, 'aqh2so4 mismatch at column ', (i-1)*j, ' and mode ', l
              passed = .false.
            end if
          end do
        end do
        if (.not. check_close(calculated(i)%aqso4_h2o2(j), expected(i)%aqso4_h2o2(j))) then
          print *, 'aqso4_h2o2 mismatch at column ', (i-1)*j
          passed = .false.
        end if
        if (.not. check_close(calculated(i)%aqso4_o3(j), expected(i)%aqso4_o3(j))) then
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  logical function check_close(a, b)

    real(r8), intent(in) :: a, b

    check_close = abs(a - b) < (abs(a) + abs(b))/2.0_r8 * 1.0e-13_r8
