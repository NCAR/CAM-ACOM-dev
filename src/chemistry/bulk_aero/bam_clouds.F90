...
module bam_clouds
  ...
  use cloud_utilities,  only : cldaero_conc_t, cldaero_allocate, cldaero_deallocate
  ...
  integer, parameter, public :: bam_id = 1
...
!----------------------------------------------------------------------------------
  subroutine sox_cldaero_update( state, &
    ...
    ! The aqueous chemistry variables are intent out, but they are not
    ! actually calculated in this subroutine.
